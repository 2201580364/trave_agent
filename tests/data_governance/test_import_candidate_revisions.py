...
def test_candidate_import_is_idempotent_and_keeps_revisions_candidate(tmp_path: Path) -> None:
    ...
    assert (
        "candidate revisions: imported=72, updated=0, skipped=0, "
        "relations_imported=15, dry_run=False" in first.stdout
    )
    ...
    assert (
        "candidate revisions: imported=0, updated=0, skipped=72, "
        "relations_imported=0, dry_run=False" in second.stdout
    )
    assert _counts(database) == (72, 72, 72)
...
def test_candidate_import_dry_run_does_not_write_rows(tmp_path: Path) -> None:
    ...
    assert (
        "candidate revisions: imported=72, updated=0, skipped=0, relations_imported=0, dry_run=True"
        in result.stdout
    )
...
def test_candidate_import_supports_controlled_batch_limit(tmp_path: Path) -> None:
    ...
    assert (
        "candidate revisions: imported=12, updated=0, skipped=0, "
        "relations_imported=6, dry_run=False" in result.stdout
    )
...
def test_candidate_import_supports_explicit_candidate_ids(tmp_path: Path) -> None:
    ...
    assert (
        "candidate revisions: imported=2, updated=0, skipped=0, relations_imported=0, dry_run=False"
        in result.stdout
    )
    assert _counts(database) == (2, 2, 2)


def test_candidate_import_repairs_only_pristine_legacy_metadata(tmp_path: Path) -> None:
    database = tmp_path / "candidate-import-legacy.db"
    _migrate(database)
    _run_selected_import(database, candidate_ids=("hz-cand-008",))
    ...
    with Session(engine) as session:
        ...
        revision.relation_review_status = "not_required"
        session.commit()

    result = _run_selected_import(database, candidate_ids=("hz-cand-008",))
    assert (
        "candidate revisions: imported=0, updated=1, skipped=0, relations_imported=0, dry_run=False"
        in result.stdout
    )
