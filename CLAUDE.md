...
**Gate 6 求解器技术验证已通过，当前处于 Gate 7 准备的 `G7-R0.2-04 杭州候选地点清单与覆盖矩阵`。** 求解器核心实现已阶段性完成，M1 对外契约已稳定并版本化；A1–A6 首轮纵向切片、R0.2-01 地点/投影 ADR、R0.2-02 来源治理和 R0.2-03 `place_catalog`/Alembic 0006 发布门禁已完成。当前只建立候选和覆盖，不直接 human_verified/published。管理侧 OM1–OM4 产品设计和 ADR-0019 已完成，但 admin-web、管理身份/API/RBAC、审核任务和管理审计尚未实现；R0.2-05-01/02 必须先补管理底座和审核工作台，再批量审核 50–75 个研究 Place。服务器 MySQL 仍停在 0002，本机禁止安装或启动 MySQL/Redis。`spike/` 是历史验证原型，不是生产实现模板。
...
3. **数据质量门禁**：Place 字段绑定 source registry/field dictionary、Revision、审核状态和发布投影；conditional 只能 staging。硬事实只从结构化数据读取，**LLM 不得生成**，管理员也不能绕过发布门禁。
...
| [docs/product/管理端功能模块设计.md](docs/product/管理端功能模块设计.md) | ★ OM1–OM4 管理侧功能、角色、页面、实施顺序和验收门禁 |
...
- 新增第三方依赖需写 ADR
- 管理端状态必须分开报告“已设计/已实现/已部署/已验收”；页面存在或数据库表存在都不等于 OM1 完成
...
1. 完成 G7-R0.2-04 杭州候选地点清单与区域/类别/人群/天气/夜间覆盖矩阵，只记录候选和重复线索。
2. 进入 R0.2-05-01/02，实现 OM1 管理身份、管理 API/RBAC、结构化审计和地点审核工作台 P0。
3. 通过管理端执行 R0.2-05-03 的采集、归一、去重、冲突裁决和 50–75 个研究 Place 人工审核，再进入按需 OD 和发布。
