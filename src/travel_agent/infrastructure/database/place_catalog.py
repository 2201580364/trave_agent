...
from .place_catalog_ports import PlaceCatalogReadPort
...
class SqlAlchemyPlaceCatalogRepository:
    ...
    def __init__(self, session: Session) -> None:
        from .place_catalog_reads import SqlAlchemyPlaceCatalogReadRepository

        self._session = session
        self._reads: PlaceCatalogReadPort = SqlAlchemyPlaceCatalogReadRepository(session)
        from .place_catalog_sources import SqlAlchemyPlaceCatalogSourceRepository

        self._sources = SqlAlchemyPlaceCatalogSourceRepository(self)
        from .place_catalog_evidence import SqlAlchemyPlaceCatalogEvidenceRepository

        self._evidence = SqlAlchemyPlaceCatalogEvidenceRepository(self)
        from .place_catalog_review import SqlAlchemyPlaceCatalogReviewRepository

        self._review = SqlAlchemyPlaceCatalogReviewRepository(self)
        from .place_catalog_publication import SqlAlchemyPlaceCatalogPublicationRepository

        self._publication = SqlAlchemyPlaceCatalogPublicationRepository(self)
    ...
    def get_publication_batch(self, batch_id: str) -> PublicationBatch | None:
        return self._reads.get_publication_batch(batch_id)

    def get_research_snapshot(self, snapshot_id: str) -> ResearchSnapshot | None:
        return self._reads.get_research_snapshot(snapshot_id)

    def list_research_snapshots(
        self, *, city_id: str | None = None, limit: int = 50, offset: int = 0
    ) -> tuple[ResearchSnapshot, ...]:
        return self._reads.list_research_snapshots(city_id=city_id, limit=limit, offset=offset)
    ...
    def load_revision_evidence(self, place_revision_id: str) -> PlaceRevisionEvidence | None:
        return self._reads.load_revision_evidence(place_revision_id)
