...
- 日期：2026-08-29
- 产品里程碑：M1
- 管理侧里程碑：OM1（产品设计与首版实现已阶段性完成，规模化真实目录与受控部署待执行）
- 设计阶段：A2
...
| O17 | 中国法定节假日历同步、年度版本、任务状态、异常和影响查询 | OM1 | 已实现（后端与迁移已落地，见 CURRENT.md） | G7-R0.2-09 官方来源、AI 抽取、确定性门禁和同步 Job 实现 |
...
- O17 正常同步只要求选择年份；公告 URL、日期、JSON、内容哈希和版本号不作为业务人员输入。未发布、暂时不可用和需要处理使用不同中文状态，不只用颜色或技术码区分。
...
- O08 固定展示完整性检查、版本差异、审核理由和通过/驳回操作；审核任务表格必须直接提供“查看地点详情”入口，按 `place_revision_id` 进入对应 Revision 详情，不能要求 reviewer 先展开任务或手工复制编号；展开行可继续承载决定历史和快捷审核操作；
...
| OM1 地点数据治理管理端 | 已完成产品级信息架构 | O00–O09/O15/O16 已进入实现与真实数据验收；O17 已设计并登记为 G7-R0.2-09，尚无可运行页面 |
