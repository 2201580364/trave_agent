...
- 产品里程碑：M1
- 设计阶段：A2
- 上游输入：功能模块设计 V3.0（M1 详细范围）、ADR-0009、M1 求解器契约
- 设计目标：以最少操作完成首次生成，同时保留透明、可理解、可调整的控制入口
...
| P08 | 分享预览 | `/trips/:tripId/share` | M1 | P1 | 否 | 6.4.* |
...
P1 页面。匿名用户只显示当前设备可访问的行程；登录用户显示账号行程。列表按最近更新排序，状态包括草稿、已生成、有未排入、生成失败。不得把失败行程伪装成正常卡片。
...
| O10–O14 | 内容治理后台 | M3 | 信息架构 | 权限、审核状态和申诉流程确认 |
...
| 内容治理后台信息架构 | 已登记 | M3 讨论区上线前必须详细设计和实现 |
