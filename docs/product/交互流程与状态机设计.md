...
- 设计阶段：A3
- 上游输入：功能模块设计 V3.5（M1 详细范围）、管理端功能模块设计 V1.0、信息架构与 UI 设计 V1.4、ADR-0009、ADR-0018、ADR-0019、M1 求解器契约
...
idle（未选择，提交禁用）
→ rating_selected
→ 非 reasonable 时按需选择 problem_types
→ 可选输入 comment
→ submitting
→ submitted / deduplicated / submit_failed
```

- 反馈关联准确的 `principal_id + trip_id + revision_id`；当前 UI 只在当前 Revision 展示，API 可接受主体拥有的历史 Revision；
- 客户端按规范化后的 `rating + 稳定排序 problem_types + trim(comment)` 计算 payload fingerprint；同一 fingerprint 的网络重试沿用同一 `feedback_intent_id`；
- `reasonable` 清空问题类型；`neutral/unreasonable` 可多选问题类型；文字可选，最多 500 字；
- `submitting` 时按钮 loading 且真实禁用；网络失败进入 `submit_failed`，保留评价、原因、文字和 intent，允许原载荷稳定重试；
- 服务端返回 `reused=true, deduplicated=false` 表示同一 intent 幂等重试；返回 `reused=true, deduplicated=true` 表示该目标已有首份反馈；两者都进入只读成功态；
- 同一 intent 改用于不同载荷进入 `feedback_intent_conflict`，不能覆盖原反馈；
- A6-9.4 首版同主体/Revision/目标只保留首份，不提供编辑状态或“再次提交覆盖”入口；
- 反馈成功不自动跳离结果页，不触发重求解，也不公开为景点评分或评论。

### 12.2 节点反馈

```text
idle
→ like（自动 arrangement_good）
   或 dislike（可选一个负面原因）
→ 可选输入 comment
→ submitting
→ submitted / deduplicated / submit_failed
...
- 节点必须存在于该 Revision 的不可变 `result_snapshot.days[].nodes[]`；node 不属于 Revision、Revision 不属于 Trip 或越权均统一为 404；
- fingerprint 包括 `rating + reason_code + trim(comment)`，失败恢复和 intent 规则与整体反馈一致；
- 节点成功态只替换该节点反馈控件，不影响其他节点、替换景点按钮或整体反馈；
- 历史 Revision 维持只读，不显示节点反馈入口。
...
→ 点击“分享计划”
→ P08 阅读隐私边界
→ 客户端生成本次 plan_share_intent_id
→ POST 创建并发布不可变 plan-share-v1
→ 展示 simple 模板计划卡
→ 复制公开链接或打开访客预览
```

- 同一页面实例的双击/网络重试沿用同一 `plan_share_intent_id`；服务端返回同一分享对象和 token；
- 同一 intent 改用于其他 Trip、Revision 或模板时进入 `intent_conflict`，不能覆盖原分享；
- 分享对象在创建时固化当前 Revision 的脱敏快照，原行程后续产生 Revision 3 不修改已分享的 Revision 2；
- 状态至少区分 `explaining → creating → published` 和 `create_failed`；生成失败不影响原 Trip/Revision，页面保留说明并允许重试；
- 首版“复制分享链接”使用安全公开 token；不把匿名 access token 放入链接、卡片、日志或错误消息；
- 当前首版完成到“安全链接 + HTML/CSS 模板卡片”，尚未调用微信原生分享，也未导出图片或二维码。
...
公开读取不要求身份，响应 `Cache-Control: no-store`，且不回显 token 或 Trip/Revision/主体等内部标识。访问状态至少区分 `loading → visible`、`not_found` 和 `copying → copied/copy_failed`。

“以此为参考新建行程”不是复制原行程：

1. 访客无会话时创建自己的匿名会话；
2. 服务端验证公开 token，只复制城市和景点集合；
3. 新草稿的 `travel_facts=null`、时段偏好为空；
4. 进入 P01，由访客重新确认日期、到达、离开、节奏和同行人群；
5. 后续生成新的 GenerationIntent/Trip/Revision，与原作者资源完全分离。

不能用分享 token 修改原行程，也不能暴露匿名访问 token、精确票号、班次/站点、交通锚点、内部 node/attraction ID、坐标、OD 明细、私人备注或未来旅行小记。计划分享、M2 旅程回顾和 M4 自动游记必须继续使用不同资源、token 密钥、默认隐私和事件语义。
...
| IF-26 | 管理员登录、超时和权限拒绝 | OM1 | 核心边界 | AdminActor、RBAC、管理会话 |
| IF-27 | candidate Revision 编辑与送审 | OM1 | 核心状态机 | PlaceRevision、乐观锁、来源证据 |
| IF-28 | Revision 审核通过与驳回 | OM1 | 核心状态机 | ReviewTask、ReviewDecision、角色分离 |
| IF-29 | 发布门、研究快照与退役 | OM1 | 流程边界 | publish_projection、snapshot、审计 |
| IF-30 | 来源冲突与地点重叠裁决 | OM1 | 流程边界 | Conflict、PlaceRelation、ExclusionGroup |
...
→ authenticated
  ├── 会话到期 → expired → 重新登录
  ├── 主动退出 → revoked
  └── 角色不足 → 保持 authenticated + 当前操作返回 forbidden
```

- 普通用户/匿名 token 访问 `/api/v1/admin` 不进入管理会话；
- 页面路由保护只改善体验，服务端 RBAC 才是权限事实；
- 会话过期时保留本地未提交表单草稿，但重新认证后必须重新读取服务端 Revision 和版本号；
- 角色变化、登录失败、会话撤销和高风险权限拒绝产生安全审计，不在响应中泄露其他管理员信息。

## 23.4 IF-27/IF-28 Revision 编辑与审核

PlaceRevision 业务状态保持：

```text
candidate → human_verified → published → retired
```

审核任务使用独立工作流，不把“正在审核”伪装成新的 PlaceRevision 生命周期：

```text
draft
→ ready_for_review
→ in_review
  ├── request_changes → changes_requested → editor_resubmit → ready_for_review
  └── approve → approved/closed + PlaceRevision=human_verified
```

规则：

- data_editor 保存时创建/更新 candidate Revision，不可选择 human_verified；
- 送审动作创建或重用开放审核任务（`ready_for_review`），可以提供页面级缺口提示，但当前服务端不把送审当作完整门禁；完整门禁的权威判断在 reviewer 的 Revision 级 `approve`；
- reviewer 必须按基础事实与来源、Geometry、AccessPoint、TimeRule、Closure、DateException、Relation 的顺序查看并逐项处理 active 证据；
- 审核人必须查看基准 Revision 差异和发布阻断项；
- reviewer 从 O08 审核队列进入 Revision 详情时，路由必须保留审核任务上下文；详情页返回 O08“地点审核”，并显示 Revision 级通过/退回/关闭操作。候选清单等普通入口不带该上下文，不显示审核操作；证据级审核按钮同样只在审核上下文且具备 reviewer 权限时出现；
- 驳回必须选择稳定 reason code，可附非敏感说明；
- 证据编辑、停用、来源冲突裁决和关系裁决都会递增 Revision 版本、清除既有审核/求解资格，并要求重新送审；旧版本的证据通过结论不得带入新版本；
- `human_verified` 只能由 reviewer 的 Revision 级 `approve` 事务产生；不能由通用 PATCH、前端状态或 SQL 直接设置；
- 审核通过和 Revision 状态更新在一个事务中完成并生成 ReviewDecision/AuditEvent；
- `expected_revision/version` 冲突返回最新状态，不能覆盖另一审核人的决定；
- 已 published 的事实变更从新 candidate Revision 开始。

## 23.5 IF-29 发布、快照与退役

```text
human_verified Revision
→ 运行 publication gate
  ├── rejected：展示稳定 reason codes，状态不变
  └── passed：展示影响预览
       → data_publisher 确认理由
       → publish_projection 原子发布
       → 可选加入 publication batch
       → 生成/更新 immutable research snapshot
       → 写入 AuditEvent
```

- 管理端不得通过通用 PATCH 把状态改为 published；
- 重复提交同一 publication intent 返回同一终态，不重复生成 Projection/快照；
- 发布批次部分失败时逐项报告，未通过项保持原状态，不能显示“全部成功”；
- 退役不物理删除旧 Revision，不改写历史 TripRevision 和 locked environment；
- 回滚只影响新请求使用的当前快照指针，并记录原因、操作者和影响范围。
...
→ 并排查看证据、地图、时长和入口
→ reviewer 选择裁决类型与理由
→ 写入结构化决定/PlaceRelation/ExclusionGroup
→ 重新运行完整性和发布门
```

裁决不删除未采用证据。飞来峰、灵隐寺等对象可以同时存在于目录，但在选择空间和求解投影前必须明确 contains/part_of/overlaps 或互斥语义；未裁决保持发布阻断。
...
- [x] 同步未扩大 M1 首个纵向切片。
- [x] IF-26–IF-30 已登记 OM1 管理身份、编辑、审核、发布和裁决流程。
- [x] 审核任务状态与 PlaceRevision 生命周期分离，管理 UI 不能直接写 published。
