...
3. 在实际研究数据库执行并核对当前代码要求的 Alembic revision。当前要求为 `0007_admin_identity_audit`；
...
数据和部署详细前置见 [`gate7-data-deployment-readiness-plan.md`](gate7-data-deployment-readiness-plan.md)。服务器验证报告仍记录为 `0002_anonymous_identity`，而当前应用要求 `0007_admin_identity_audit`。仍须在 R0.3 先备份、再按 0003→0004→0005→0006→0007 顺序部署和验证；本规范和工具不会自行连接服务器或执行迁移。
...
py -3.12 scripts/lock_gate7_environment.py `
  ...
  --database-revision 0007_admin_identity_audit `
