...
| # | 任务 | 拆分对象 → 目标 | 状态 |
|---|---|---|---|
| S7-1 | `application/admin/review.py` → 按子域拆分审核用例与窄 Protocol，保留显式兼容门面 | 11 个职责模块 + 兼容门面 | 已完成（2026-09-13；门面保留公共签名与事务装配，约 1,086 行） |
| S7-2 | `interfaces/http/admin.py` → 按 O04/O05/O07-O08/O09/O17 路由分模块 + 模型归位 | 5 个路由模块 | 已完成（2026-09-13） |
| S7-3 | `infrastructure/database/place_catalog.py` → 按聚合根拆仓储 | 5 个仓储边界 + 兼容门面 | 已完成（2026-09-13） |
| S7-4 | `admin-web/src/pages/RevisionDetailsPage.tsx` → 按证据面板与资料卡拆组件 | 10 个组件 | 已完成（2026-09-13；资料卡、证据面板、发布准备与操作区边界完成） |
| S7-5 | `GET /api/v1/trips` N+1 修复：仓储层批量计数接口 | app.py:461-480 | 已完成（2026-09-13） |
...
- **M-d（S7+S8 完成）**：进行中（S7 已完成；S8-1/2/3 阶段交付，响应类型迁移与 S8-4 未完成）——代码健康达标 + 用户端有回归保护，**具备进入 R0.3 部署与 M2 扩展的基座**
...
### 2026-09-11 业务改造衔接（H3/C2，ADR-0025）
 ...
 O05 多固定场次自动选择与候选时间规则删除已进入 dev（6e9e463）；本轮按 ADR-0026 完成固定场次晚入场规则、三地点两轮生命周期和用户端求解浏览器验收。实现与文档待用户手动提交；剩余问题为平湖秋月真实事实补证和全仓 mypy 存量债。此为独立业务能力切片，不改变 S8 响应类型迁移、S8-4 E2E 和 S7 数据批次的未完成状态。
...
### 2026-09-16 独立求解器质量能力切片（H3 / ADR-0027）

已设计、已实现、已本地部署；十景点三日结果执行全行程负载搜索、固定场次日用餐/时长精修，适配器保留营业星期/季节和非show航次，撤销错误合窗；邻近吸附阶段已将跨天的平湖秋月/断桥残雪恢复同日。锁定环境自动门禁通过，真实研究库/API回放与内置Chrome页面均为10安排/0未排入；Chrome 已完成三步生成、逐日切换和刷新恢复验证。版本v8/parameters-p1-2026-09-16b。详见 `docs/test/reports/solver-quality-20260916.md`；提交由用户手动执行。
