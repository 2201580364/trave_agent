...
提交边界按“可验收能力域/里程碑切片”划分，而不是按单个 API、字段或页面按钮划分。
一个提交至少应同时包含对应的后端契约、权限/审计、前端流程（如适用）、自动化测试、
验收证据和状态文档；只有完成该能力域的最小闭环后才判断适合提交。未完成的连续工作保留
在同一工作区，避免产生难以回滚和审阅的过细提交。
...
- 更新时间：2026-09-02
- 当前已提交基线：`579f208 feat(admin-web): 增加已发布地点目录并升级管理端整体UI`；O07 规则审计、已发布目录进入用户端、关系/求解资格修复、管理列表查询与业务字段改造均已提交，未包含凭证或密钥；本轮仅更新状态文档和路线图
- 产品里程碑：`M1 — 行程骨架验证`
...
- 当前阶段：`Gate 7 — G7-R0.2-08 O05 节假日规则修复与收口（当前节点）`
- 当前任务：完成 O05 登录后的浏览器最终视觉验收和提交；随后进入 G7-R0.2-09 中国法定节假日历自动同步与版本治理，再继续首批 12 条真实 candidate 的批量审核、Projection 和发布回归。本机继续禁止部署或启动 Redis/MySQL 服务
- 总体判断：A6-9.1 至 A6-9.4、G7-R0.1、R0.2-01～04、R0.2-05-01A、R0.2-05-01B、R0.2-05-02 和 O09 发布中心首版已阶段性完成；O05 节假日规则代码与自动化已完成，O17 仅完成 ADR/产品/契约设计。仍缺 O05 最终浏览器验收、O17 实现、真实 candidate 批量审核、完整 Projection 生成结果、OD 扩容、服务器部署和 dry run，不能宣称 Gate 7、M1 MVP 或稳定生产已完成
...
| 管理端产品设计与后端底座 | OM1–OM4 路线完成；O00–O16 已进入阶段性实现，O17 已完成设计登记 | ADR-0019、ADR-0020、ADR-0021、ADM-1～ADM-13、O00–O17；O17 同步服务、Job、Tool 和页面尚未实现 |
| A2 信息架构与 UI 设计 | M1 用户端详细设计；OM1 管理端产品级 IA 已同步 O17 | 保留用户端三模式；管理端新增 O17 中国法定节假日历入口与状态规则，见 `docs/product/信息架构与UI设计.md` |
...
| Gate 7 R0.2-04 候选清单与覆盖矩阵 | 完成 | 72 个 candidate、11 区域、9 类别、18 夜间/固定时段、28 室内/雨天、24 非点状候选、15 组未裁决关系线索、确定性覆盖矩阵、CLI 和 7 项测试 |
...
最新稳定技术基线：当前全量 pytest `393/393`、admin-web Vitest `34/34`、typecheck、production build 和 `git diff --check` 均通过；数据库迁移链已升级到 `0013_backfill_solver_eligibility`，服务器真实 MySQL 仍保持既有基线，未在本轮连接或迁移。普通匿名 token 无法访问管理 API；密码使用 scrypt，管理 token 只存 SHA-256；角色变化使旧会话失效；最后一个 admin_security 不可移除；管理审计与每日分级文件日志分离。Gate 6、A6、R0.2-02～04 的既有证据与 72 个 candidate 哈希均未改写。全仓 Ruff 和 strict mypy 仍有历史债务；以上仍只是工程和验证准备证据，不证明 H3/H11 已被专家或用户证实。
...
- 状态：G7-R0.1、R0.2-01～04、R0.2-05-01A、R0.2-05-01B、R0.2-05-02 和 R0.2-07 首版已阶段性完成；当前执行节点为 R0.2-08，R0.2-09 已设计待实现。首个真实 `locked` 环境尚未生成，尚未开始招募或收集真实用户数据；
- 当前输入：已通过 Gate 6 的求解器证据、A6 完整浏览器纵向切片、结构化 Revision/节点反馈能力，以及单条真实地点的人工核验—发布—用户可见性闭环；
...
- 下一产物：先完成 R0.2-08 O05 浏览器验收与提交，再实施 R0.2-09 中国法定节假日历自动同步；随后继续首批 12 条真实杭州地点审核/Projection/发布小批次，完成 R0.2-06 OD 子图并扩展到 50–75 个研究地点，再进入统一 Compose 的服务器 H5/HTTPS、locked manifest 与内部 dry run；
...
- O05 时间证据已实现 candidate 新增、编辑、软停用/重新启用与 reviewer 逐项通过/驳回；写入递增 Revision 版本、清除求解/审核资格并写入管理审计；Revision 级 approve 会检查全部 active 时间证据均为 `human_verified`；
- 当前剩余：O05 指定日期解析预览，O01 待办摘要、O06 来源冲突、O07 关系裁决及 72 个 candidate 批量审核仍未完成；Revision 差异视图、证据依赖复制策略和独立 research snapshot 仍属于后续切片；本机不部署 MySQL/Redis。

- 下一切片：由后端提供 O05 指定日期解析预览，把日期例外覆盖、多天闭馆、跨午夜和固定场次同日歧义以稳定结果返回；随后进入 O01 管理首页待办摘要，再推进 O06/O07。
...
### 本轮更新（2026-08-30，O05 时间证据写入与审核）

- 当前里程碑：M1；当前 Gate：G7 / R0.2-05-02 OM1 O05 地点审核工作台。
- 本轮完成：三类 O05 时间证据 candidate CRUD、软停用/重新启用、Revision 版本递增、幂等审计和 reviewer 逐项通过/驳回；Revision approve 已纳入 active 时间证据完整性门禁；admin-web 新增三类编辑表单和按权限显示的操作列。
...
- 当前进度：O05 时间证据写入、reviewer 逐项审核、指定日期解析预览、O01 管理首页待办摘要、O06 来源冲突只读面/裁决/审核门禁，以及 O07 PlaceRelation evidence 装配、来源闭包和逐项审核已完成，达到可提交状态。
- 遗留问题：O05 指定日期解析预览，O01、O06、O07、发布中心和批量审核仍属于后续节点；SQLite datetime adapter 警告和全仓历史 Ruff 不属于本轮阻断。
- 下一步：提交本轮 OM1 地点审核工作台完整闭环；随后进入 O09/发布中心后端批次与研究快照治理。
...
| 3.7 | OM1–OM4 管理端产品设计 | 管理功能、页面、流程、架构、API 和逻辑数据模型 | 设计完成；OM1 身份/API/审计、候选/审核/发布工作台和已发布目录已阶段性实现并提交；规模化真实目录与受控部署待执行 |
| 4 | A4 | 应用代码架构、模块边界与依赖规则 | 已完成 |
| 5 | A5 | API 与持久化数据模型同步 | 已完成 |
| 6 | A6 | 首个浏览器可操作纵向切片 | 已完成（100% 仅指技术纵向切片；替换、我的行程、计划分享、结构化反馈均通过 Chrome 验收） |
| 7 | G7 | 真实专家评审与用户验证 | R0.2-01～04、05-01A、05-01B、05-02 已阶段性完成；单条真实发布闭环已验证，当前推进 R0.2-07 数据隔离与批量真实目录，后续为 OD、R0.3 部署、R0.4 dry run、招募和真实证据 |
