...
- 当前阶段：`A6-9.1 M1 景点替换与新 Revision 工程实现及自动化验证已完成，浏览器复验待补；下一主实现为 A6-9.2“我的行程”`
- 当前任务：实现当前匿名设备可访问的“我的行程”列表、最新 Revision 恢复和历史版本入口；随后补计划分享与结构化反馈。FastAPI/Taro 生产发布、集中监控和异地备份仍是独立发布门禁。本机继续禁止部署或启动 Redis/MySQL 服务
- 总体判断：后端 HTTP、SQLAlchemy 持久化、前端六页面、H5 production build、既有 Chrome 门禁、日内节奏修订、高德真实 Provider 和和风真实三日预报已形成可操作技术纵向闭环；A6-9.1 新增结果页单景点替换、修订 Intent 血缘、同 Trip Revision 递增、并发基准版本保护和旧 Revision 回放。当前仍缺“我的行程”、计划分享、结构化反馈、正式应用服务发布、集中监控、异地备份、真实多机网络分区验证、餐厅节点与 Gate 7，不能宣称稳定生产可用或已完成 M1 MVP

M1 产品闭环核对：杭州单城入口、一键生成、结果保存/恢复已完成；“替换景点→完整重求解→新 Revision”已完成工程实现和自动化验证，待真实浏览器复验；到离交通、节奏/同行人群、景点筛选仍是首切片简化实现；“我的行程”、计划分享和结构化反馈尚未实现。当前 M1 产品收口主队列工程完成 1/4，完成剩余能力并进入 Gate 7 前不能直接跳转到 M2。
...
| A6-9.1 景点替换与新 Revision | 工程完成，浏览器复验待补 | 结果页替换入口、完整重求解、同 Trip Revision 递增、幂等与并发冲突保护、Alembic 0003 |

最新稳定技术基线：全量 277 项测试通过；Golden 8/8；降级 8/8；杭州公开攻略综合接近度 0.975；数据校验 7/7；性能 PERF-12-3 P95 14.94ms、PERF-20-7 P95 19.32ms，均通过。当前机器契约为 `solver-p1-v2 / constraints-p1-v5 / parameters-p1-2026-08-26`，结果结构为 `trip-result-v2`。本轮前端 TypeScript、H5 production build、改动文件 Ruff 和 SQLite Alembic 0003 升级通过；HTTP 自动化验证替换后仍使用同一 Trip、Revision 1→2、旧版本可读、重复提交幂等、过期基准返回 409，数据库测试验证 Trip 当前版本条件更新只能有一个发布者成功。浏览器控制层本轮没有发现可用实例，因此新增替换页面尚未完成真实点击复核，不夸大为 Chrome 已验收。入口约 304 KiB 的既有性能警告仍保留为 P1 优化项。全仓 Ruff 与 strict mypy 历史债仍未清零。该证据只证明技术可行性，不证明 H3 已被专家或用户证实。
...
- 状态：已完成
- 核心产物：`docs/specs/api-contract.md` V2.2、`docs/specs/data-model.md` V2.2
...
- 已完成：A6-1 至 A6-8.2 技术纵向切片与真实基础设施；A6-9.1 景点替换和新 Revision 工程闭环，新增页面浏览器复验待补
...
- 下一步：实现 M1“我的行程”列表、最新 Revision 恢复和历史修订入口；提交前补新增替换页面的真实浏览器复验
...
- 全量 pytest 由 275 增至 277 项通过；前端 TypeScript、H5 production build 和改动文件 Ruff 通过；入口仍约 304 KiB，保留既有 P1 性能项；
- 本地 FastAPI readiness 与 H5 页面均返回 200，但浏览器控制层本轮没有发现可用实例，故新增页面的真实点击和响应式复验登记为提交前门禁，不标记为已完成 Chrome 验收；临时 API/H5 进程已停止，无 8000/10086 监听残留；
- 下一主实现转入 A6-9.2“我的行程”列表和历史恢复入口，同时保留替换页真实浏览器复验门禁。M1 产品收口主队列按“替换、我的行程、分享、反馈”统计为工程完成 1/4。
...
| 6 | A6 | 首个浏览器可操作纵向切片 | 进行中（约 99%；景点替换/新 Revision 已完成；下一步“我的行程”与历史恢复） |
...
- Alembic 0003 已在本地升级和自动化测试通过，但服务器真实 MySQL 仍是已验收的 0002 基线；下一次应用发布前必须以迁移账号执行 0003 并重新验证 readiness、备份与隔离恢复；
...
- Taro/React P00–P04 与景点替换页、依赖锁定、TypeScript、H5 production build 已完成；既有 P00–P04 Chrome 移动端/桌面端门禁已通过，但新增替换页尚待浏览器控制实例可用后复验；多浏览器、弱网、长时会话和生产部署验证尚未完成；
...
- 分享卡片尚未实现；
- “我的行程”列表、最新 Revision 恢复和历史修订查看入口尚未实现；
