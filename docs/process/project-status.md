...
- 当前阶段：`Gate 7 — G7-R0.2-07 OM1 研究数据审核、发布与用户可见性收口（当前节点）`
- 当前任务：本轮管理列表查询与业务字段能力已完成；提交后继续发布数据与验收 fixture 隔离、历史乱码数据清理、受控部署前数据准备，为 G7 真实人工评测构建可用目录；研究数据不得通过 SQL 手工改状态，未审核依赖继续阻断发布；本机继续禁止部署或启动 Redis/MySQL 服务
...
- 状态：G7-R0.1、R0.2-01～04、R0.2-05-01A、R0.2-05-01B 和 R0.2-05-02 已阶段性完成；当前执行节点为 R0.2-07 真实研究目录准备。首个真实 `locked` 环境尚未生成，尚未开始招募或收集真实用户数据；
...
- 下一产物：先建立 local/dev/test/research 数据边界并隔离 Browser E2E published fixture，再以 10–15 个真实杭州地点执行审核/Projection/发布小批次；随后完成 R0.2-06 OD 子图、扩展到 50–75 个研究地点，再进入统一 Compose 的服务器 H5/HTTPS、locked manifest 与内部 dry run；
