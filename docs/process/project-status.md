...
`G7-R0.2-05-02` 正在实施：O05 时间证据已完成 candidate CRUD、逐项审核和指定日期解析预览纵向切片；O01 管理首页已接入真实 Revision/审核任务聚合。解析由后端按已核验证据稳定计算，日期例外优先于周规则并返回固定 reason codes；前端仅展示结果。O06 来源、O07 关系和批量审核仍待后续推进。本机 SQLite 已迁移到 `0010_place_revision_version`，服务器 MySQL/Redis 未连接。
...
- 当前进度：O05 时间证据写入、reviewer 逐项审核、指定日期解析预览和 O01 管理首页待办摘要已完成，达到可提交状态。
- 遗留问题：O05 指定日期解析预览，O01、O06、O07、发布中心和批量审核仍属于后续节点；SQLite datetime adapter 警告和全仓历史 Ruff 不属于本轮阻断。
- 下一步：提交本轮 O01/O05 管理工作台改动；随后实现 O06 来源冲突治理。
