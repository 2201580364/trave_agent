...
提交记录：本能力域已提交为 `a96d0e7 feat(gate7): 完成节假日历同步与审核治理闭环`。
...
用户已完成节假日历同步功能人工验收：同步任务创建、AI/系统执行过程流、预览结果查看、预览数据调整、确定性校验以及确认入库发布路径均已实际走通，未发现新的业务阻断。结合后端全量 `428/428`、管理端全量 `41/41`、类型检查、生产构建和 API readiness 验证，O17 已达到本轮“大模块提交”条件。当前工作区同时包含 O06 来源冲突误判修复和 O07 reviewer 审核入口修复，建议作为同一 G7 管理审核与节假日同步能力域提交；提交后进入 G7-R0.3 受控研究数据扩充与真实多地点回归。
...
当前节点已转为 `M1 后段 / Gate 7 / OM1 / G7-R0.3`。O17 的真实配置人工验收已完成，Calendar 版本影响查询已实现：`0015_holiday_exception_provenance` 为物化日期例外保存日历版本来源，接口返回日期差异、受影响地点，并明确排除无来源历史记录。下一阶段进入受控研究数据扩充和多地点回归。
