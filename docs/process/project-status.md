...
`G7-R0.2-05-02` 正在实施：O05 时间证据已完成 candidate CRUD、逐项审核和指定日期解析预览纵向切片。解析由后端按已核验证据稳定计算，日期例外优先于周规则并返回固定 reason codes；前端仅展示结果。O01 待办摘要、O06 来源、O07 关系和批量审核仍待后续推进。本机 SQLite 已迁移到 `0010_place_revision_version`，服务器 MySQL/Redis 未连接。
