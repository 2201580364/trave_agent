...
- 更新时间：2026-09-16（ADR-0026 固定场次晚入场、三地点生命周期验收与 OD 近邻分天修复）
...
- 本轮本地真实浏览器验收：`http://127.0.0.1:10086` 首页显示登录/注册、游客提示与自绘操作控件；游客会话读取 13 条已发布景点，选中 3 条并成功生成 3 日行程（3 已安排、0 未排入）。后端全量 `pytest` 在 `PYTHONUTF8=1` 下通过，前端 typecheck/build/vitest 通过。
- 本轮关系约束：已发布且 `human_verified` 的 `selection_exclusion_groups` 随数据库目录加载到 `PublishedAttraction`；选点接口与 `ProductionSolverGateway` 双层拒绝同组重复选择，并新增数据库/求解器回归。
...
- `SOLVER-DAY-001` 已解决：Step 1 的 OD 聚类时长平衡曾拆散发布 OD 中的强近邻；现保留对称 OD ≤10 分钟的近邻对，新增杭州七景点回归。浏览器全量复测仍待服务重启后执行。
