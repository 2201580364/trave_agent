...
1. 继续 S7-1：继续S7-2：拆O17节假日同步路由并完成全量回归；O04/O05/O07/O08/O09已拆出；S7-1业务职责已拆出，门面显式兼容转发约1027行；准备度/来源/几何/关系/时间职责已拆出；本地原批次剩余 027/052 两条 candidate，048 已发布场次的最晚入场晚于开始 10 分钟，需按 ADR-0025 核对。
...
| S7-3 仓储职责拆分（H3，已提交 bca173f）；S7-4 管理端详情页拆分进行中（O04 几何/访问点完整组件迁移已提交；O05 时间证据完整组件迁移已提交；O07 关系证据完整组件迁移已提交；O06 来源冲突与验证汇总组件已提交；发布阻断计算组件已提交；修订操作区组件完成待提交） | dev | `infrastructure/database/{place_catalog,place_catalog_reads,place_catalog_ports}.py`、CURRENT/改造计划/本月归档 | 进行中 |
