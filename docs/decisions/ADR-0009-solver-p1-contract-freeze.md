...
因此冻结 M1 对外契约。冻结不禁止修复缺陷和内部重构，而是要求任何会改变输入、输出、状态、拒绝码、约束或默认参数的修改显式升级版本并重新通过 Gate 6。
...
M1 使用确定性主方案，不实现 ADR-0006 的替代候选池。
...
契约冻结后，求解器核心实现标记为“阶段性完成”，主线停止无边界地主动增加规则，下一阶段正式转入 M1 功能模块、信息架构和页面设计。这不表示代码或未来能力永久冻结：应用集成、真实数据、专家评审或用户验证发现缺陷时可以继续修复和演进；若改变已冻结行为，则按本 ADR 评审并升级相应版本。
