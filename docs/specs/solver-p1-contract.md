...
- **日期**：2026-08-26
- **依据**：ADR-0009、ADR-0010、ADR-0011、ADR-0012、ADR-0013
- **关联假设**：H3、H7
...
OD duration rebalance max symmetric penalty = 10min
...
版本升级为 `constraints-p1-v8 / parameters-p1-2026-09-16b`，请求/结果 schema 不变，历史 Revision 不覆盖。默认分天的初始数量均衡不作为最终数量差承诺；可行性恢复后按用餐、完整时长、晚间余量、强近邻同日、单景点日规避及按日容量归一化的游览加缓冲交通负载优化。候选必须全量守恒并重新通过硬校验。
