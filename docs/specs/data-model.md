...
## show 多场次与候选删除（ADR-0025，H3/C2）

show 至少维护一条 active 且人工核验的固定场次，允许多条；每条必须满足开始时间 < 结束时间，填写最晚入场时须满足最晚入场 < 结束时间，允许开场后入场（ADR-0026）。实际观看开始为 max(入场, 开场)，须早于结束；固定场次不以建议时长的 60% 硬拦。来源、星期和日期范围必须有效。求解器从离散场次中选择可行的一条，不把间隔合并为开放时间。`session_override` 仅替代对应日期场次，关闭例外仍优先。

投影 JSON 的可选 `fixed_sessions` 固化场次与来源、日期限定及开放规则并参与哈希，无数据库迁移。候选入口删除时间规则仅删除该 candidate 的子记录，原子递增版本、重置资格并保留审计。审核入口停用保留行；停用规则不参与校验，不影响其他有效规则。
