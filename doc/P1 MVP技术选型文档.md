# 旅行AI Agent — P1 MVP 技术选型文档

**版本**：V1.1  
**文档类型**：技术选型（已确认）  
**关联文档**：旅行助手产品文档.md（V2.2）  
**状态**：全部决策已确认，待进入详细设计阶段
...
**✅ 已确认**：P1 直接用 Taro + React，同时出 H5 Web 和微信小程序。

**决策理由**：
1. P1 即可验证完整获客闭环——分享卡片嵌入小程序码 → 扫码 → 新用户注册
2. 微信生态（搜一搜、分享朋友圈）是 P1 冷启动的重要渠道
3. 虽然多端兼容调试增加 20-30% 前端工作量，但后端 FastAPI 和 Python 生态的开发效率可以挤出时间
4. 不存在"P2 迁移重写"的问题，从一开始就是多端架构
...
**✅ 已确认**：Python 3.12 + FastAPI 作为 P1 主后端，Java 暂不使用。
...
5. P1 并发量低（DAU < 1000），Python 性能完全够用

**Java 未来定位**：P2-P3 若需高并发票务预订、复杂事务管理，可拆出独立 Java 微服务。
...
**⚠️ 标注**：P2 引入住宿位置优化时，重新评估**方案 A（多车辆建模）**。住宿位置与景点日期分配是互耦的——住在哪里影响每天去哪，每天去哪反过来影响最优住宿位置。多车辆建模可以一次性求解"景点分配 + 住宿位置 + 天内排序"的全局最优。
...
| 硬约束检查通过率 | 100% | 自动化校验：开放时间、体力预算、天气标识 |
...
| 数据 | 存储 | 说明 |
|------|------|------|
| 景点基础数据（标签、坐标、开放时间、体力消耗、室内外） | **MySQL** | P1 约 80-120 条，爬虫采集 + LLM 结构化 + 人工校准 |
| 用户数据（微信 openid） | **MySQL** | 微信扫码登录，P1 用户量小 |
...
**P1 不需要。** 理由：
- P1 的空间计算（地理聚类 K-Means、OD 矩阵）都在求解器内存中完成，不依赖数据库空间扩展
- 景点坐标存 `DECIMAL(10,7)` 字段即可，查询时加载到内存由 NumPy 处理
- P2 住宿区域建议可能需要空间查询，届时再评估是否引入 PostGIS
...
| API | 免费日调用量 | P1 预估用量 | 是否够用 |
|------|------|------|------|
| 地理编码 | 5000 次/日 | < 100 次/日 | ✅ |
| 路径规划 | 5000 次/日 | 预计算 120×120 = 14400 次（一次性），日常 < 100 次/日 | ✅ 预计算后缓存 |
| 天气查询 | 100000 次/日 | < 50 次/日 | ✅ |

> 预计算 OD 矩阵：P1 1 个城市约 120 个景点，景点对 OD 矩阵为 120×120 ≈ 14400 次路径规划请求。一次性计算后存入 Redis 缓存，后续仅需增量更新。
...
| 风险 | 严重程度 | 缓解措施 |
|------|------|------|
| **Python 性能瓶颈** | 低 | P1 用户量小，FastAPI async 足够；求解器是计算密集型，Celery 异步隔离 |
...
| **单人开发 4 个月交付** | 中 | 后端 FastAPI 开发效率高；景点数据爬虫自动化减少人工标注；不做 P2+ 功能 |
| **高德 API 费用** | 低 | 初期用量小，免费额度内；OD 矩阵预计算后缓存 |
| **LLM API 不稳定** | 低 | 可插拔架构，DeepSeek 不可用时切换 Claude/GPT；行程解释非实时，可重试+降级 |
| **求解器效果不达预期** | 高 | 设置硬性质量门（硬约束 100% 通过），P1 上线前用户测试 n≥20 验证 |
| **景点数据质量** | 中 | 聚焦 1 个城市，爬虫初筛 + LLM 结构化 + 人工抽检校准三层保障 |
| **P1 范围膨胀** | 中 | 严格用 MVP 决策逻辑约束："缺了它核心假设还能验证吗？" |
| **微信登录审核** | 低 | 微信开放平台网站应用需企业资质，提前准备；小程序端登录无门槛 |
| **Puppeteer 内存开销** | 低 | 独立容器 ~300MB，P1 并发低，单实例足够；可按需扩缩 |
...
*V1.1 变更说明：所有第七节待决策事项已确认。前端改为 Taro 多端、LLM 改为可插拔架构、部署改为腾讯云、认证改为微信扫码登录、求解器确认方案 B + 天气方案 C、景点数据改为爬虫+LLM+人工校准、分享卡片改为服务端渲染。新增第十一节 P1 开发里程碑建议。*